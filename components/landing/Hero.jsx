...
import { Activity, ArrowRight, Calendar, CheckCircle2, ChevronRight, Clock, Heart, ShieldCheck, Sparkles, Star, UserCheck, Video, Zap } from "lucide-react";
          ...
          <div className="relative overflow-hidden rounded-[2.5rem] sm:rounded-[3.75rem] border border-white/90 bg-gradient-to-b from-white/95 via-white/90 to-indigo-50/70 p-8 sm:p-14 lg:p-20 shadow-[0_40px_120px_-15px_rgba(99,102,241,0.35)] backdrop-blur-3xl dark:border-slate-800 dark:bg-gradient-to-b dark:from-slate-950/95 dark:via-slate-950/90 dark:to-indigo-950/50 dark:shadow-[0_40px_120px_-15px_rgba(0,0,0,0.9)]">
            ...
            >
              ...
              <motion.div
                ...
                className={
                  "mx-auto mb-8 inline-flex items-center gap-2.5 rounded-full border border-indigo-200/90 bg-gradient-to-r from-indigo-50/95 via-purple-50/95 to-cyan-50/95 px-6 py-2.5 shadow-md shadow-indigo-500/20 backdrop-blur-xl dark:border-indigo-500/40 dark:from-slate-900/95 dark:via-indigo-950/95 dark:to-slate-900/95 " +
                  transitionBase +
                  " hover:border-indigo-300 hover:shadow-lg dark:hover:border-indigo-400/50"
                }
              >
                <Sparkles className="h-4 w-4 text-indigo-600 animate-spin dark:text-indigo-400" aria-hidden style={{ animationDuration: "8s" }} />
                <span className="text-xs font-black uppercase tracking-[0.25em] text-indigo-950 dark:text-indigo-200">
              ...
              <motion.p
                variants={motionSafe ? item : undefined}
                className="mx-auto mt-8 max-w-3xl text-pretty text-lg sm:text-xl leading-relaxed font-semibold text-slate-700 dark:text-slate-300"
              >
              ...
              <motion.div
                variants={motionSafe ? item : undefined}
                className="mx-auto mt-10 flex flex-wrap justify-center gap-3 sm:gap-3.5"
              ...
              >
                {heroCapabilityPills.map((label) => (
                  <span
                    key={label}
                    className={
                      "rounded-2xl border border-indigo-600/20 bg-indigo-600/10 px-4 py-2 text-xs font-extrabold uppercase tracking-wider text-indigo-900 shadow-sm backdrop-blur-md dark:border-indigo-400/30 dark:bg-indigo-500/15 dark:text-indigo-100 " +
                      transitionBase +
              ...
              {/* NEW: Live Interactive Doctor Availability Quick-Selector Bar */}
              <motion.div
                variants={motionSafe ? item : undefined}
                className="mx-auto mt-12 max-w-3xl rounded-3xl border border-indigo-200/80 bg-white/95 p-6 shadow-2xl shadow-indigo-500/15 backdrop-blur-2xl dark:border-slate-700/80 dark:bg-slate-900/90"
              >
                <div className="flex items-center justify-between pb-4 border-b border-indigo-100 dark:border-slate-800">
                  <div className="flex items-center gap-2">
                    <UserCheck className="h-5 w-5 text-indigo-600 dark:text-indigo-400 animate-pulse" />
                  ...
                  <span className="flex items-center gap-1.5 rounded-full bg-emerald-100/80 px-2.5 py-1 text-[10px] font-extrabold text-emerald-800 dark:bg-emerald-950/80 dark:text-emerald-300">
                    <span className="h-2 w-2 rounded-full bg-emerald-500 animate-ping" />
                    Live Doctors Online
                  </span>
                </div>
                ...
                {/* Doctor Bio Card */}
                <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-2xl bg-gradient-to-r from-indigo-50/50 to-purple-50/50 p-4 border border-indigo-100/60 dark:from-slate-800/50 dark:to-indigo-950/40 dark:border-slate-700/60">
                  <div className="flex items-center gap-3.5 text-left">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-heading font-black text-slate-900 dark:text-white text-base sm:text-lg">{activeTab.name}</h4>
                        <span className="flex items-center gap-0.5 rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-bold text-amber-800 dark:bg-amber-950/80 dark:text-amber-300">
                      ...
                      <p className="flex items-center gap-1.5 text-xs font-extrabold text-indigo-600 dark:text-indigo-400 mt-1">
                        <Clock className="h-3.5 w-3.5" /> {activeTab.time} · <span className="text-emerald-600">{activeTab.status}</span>
                      </p>
                    </div>
                  </div>
                  <Link
                    href="/register"
                    className="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-6 py-3 text-xs sm:text-sm font-extrabold text-white shadow-lg shadow-indigo-500/30 transition hover:scale-105 w-full sm:w-auto"
                  >
              ...
              >
                <Link href="/register" className={btnGradient + " group min-h-[3.75rem] min-w-[240px] gap-3 rounded-2xl px-8 text-base shadow-2xl shadow-indigo-500/40 font-extrabold transform hover:scale-[1.02]"}>
                  <Zap className="h-5 w-5 fill-white text-white animate-bounce" style={{ animationDuration: "2s" }} />
                  Start Free Portal
                  <ArrowRight className="h-5 w-5 transition group-hover:translate-x-1" aria-hidden />
                </Link>
                <Link href="/features" className={btnGhost + " min-h-[3.75rem] min-w-[200px] rounded-2xl px-8 text-base font-extrabold shadow-sm border border-slate-300/80 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800"}>
                  Explore Features
                </Link>
              </motion.div>
              ...
              <motion.div
                variants={motionSafe ? item : undefined}
                className="mx-auto mt-14 pt-8 border-t border-indigo-100 dark:border-slate-800/80 flex flex-wrap items-center justify-center gap-6 sm:gap-10 text-xs font-extrabold text-slate-600 dark:text-slate-400 uppercase tracking-widest"
              >
                <div className="flex items-center gap-2">
